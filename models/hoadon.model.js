...
HoaDon.requestExtension = async (ma_hoa_don, ngay_gia_han) => {
  const query = `
    UPDATE hoa_don
    SET ngay_gia_han = $1, da_duyet_gia_han = false
    WHERE ma_hoa_don = $2
    RETURNING *
  `;
...
HoaDon.approveExtension = async (ma_hoa_don) => {
  const query = `
    UPDATE hoa_don
    SET da_duyet_gia_han = true, han_dong_tien = ngay_gia_han
    WHERE ma_hoa_don = $1
    RETURNING *
  `;
...
HoaDon.calculateBill = async (ma_phong, chi_so_dien_moi, chi_so_nuoc_moi) => {
  try {
    // Lấy thông tin phòng
    const phongQuery = `
      SELECT p.ma_phong, p.gia_thue, p.ma_day_tro, d.gia_dien, d.gia_nuoc
      FROM phong p
      JOIN day_tro d ON p.ma_day_tro = d.ma_day
      WHERE p.ma_phong = $1
    `;
